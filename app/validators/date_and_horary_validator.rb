class DateAndHoraryValidator < ActiveModel::Validator
  def validate(record)
    ...
    if record.patient.consultations.where.not(id: record.id).where(consultation_date: record.consultation_date, horary: record.horary).present? 
      record.errors.add(:horario, 'Paciente já possui consulta agendada para este horário!')
    end
